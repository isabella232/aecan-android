...
public class CameraSource {
    ...
    public CameraSource(Activity activity, GraphicOverlay overlay, Size size) {
        this.activity = activity;
        graphicOverlay = overlay;
        requestedPreviewWidth = size.getWidth();
        requestedPreviewHeight = size.getHeight();
    ...
    public synchronized CameraSource start(SurfaceHolder surfaceHolder) throws IOException {
        ...
        requestedPreviewWidth = surfaceHolder.getSurfaceFrame().width();
        requestedPreviewHeight = surfaceHolder.getSurfaceFrame().height();
    ...
    private static SizePair selectSizePair(Camera camera, int desiredWidth, int desiredHeight) {
        ...
        System.out.println("validPreviewSizes: " + validPreviewSizes);
        ...
        float minRes = Integer.MAX_VALUE;

        for (SizePair sizePair : validPreviewSizes) {
            Size size = sizePair.previewSize();
            float diff = Math.abs(Math.abs((float) size.getWidth() / (float) desiredWidth) - Math.abs((float) size.getHeight() / (float) desiredHeight));
            System.out.println(" diff:  " + diff);
            if (diff <= minDiff) {
                if (diff < minDiff) {
                    selectedPair = sizePair;
                    minDiff = diff;
                    minRes = size.getWidth();
                } else if (diff == minDiff &( minRes >= size.getWidth() | minRes < desiredWidth/1.5)) {
                    if(size.getWidth() > desiredWidth/1.5){
                        selectedPair = sizePair;
                        minDiff = diff;
                        minRes = size.getWidth();
                    }
        ...
        System.out.println("selectedPair" + selectedPair.toString());
        return selectedPair;
    }
    ...
    private static class SizePair {
        ...
        public String toString() {
            return "SizePair{" +
                    "preview=" + preview.getHeight() + ", " + preview.getWidth() +
                    ", picture=" + picture.getHeight() + ", " + picture.getWidth() +
                    '}';
        }
    }
